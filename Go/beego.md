...
​	这里要解释清楚，就需要把动态生成的那个路由文件说清楚。首先，生成的路由文件并不是以什么 conf、properties 为后缀的配置文件的标注形式，而是 go 的源码形式，将路由配置放到内存这个过程放到 init 函数中，巧妙利用 go 的 init 机制；其次，这个文件是运行时期（`E:\Learning-Program\golib\pkg\mod\github.com\beego\beego\v2@v2.0.1\server\web\parser.go` - 113 `genRouterCode`）生成的，即若是刚刚生成的文件就是没有经过编译执行的，所以 init 函数中的内容就没有执行，即，项目相当于没有路由，所有接口就无法访问；所以，当第二次运行项目时，这个已经存在的路由文件的 init 函数才会执行，路由才会被加载到内存中

结论：编译时期，存在的动态生成的路由文件才会使路由生效，如果接口有变动，想使其生效，也是同理重启两次才行
